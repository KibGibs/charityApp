...
class DonationController extends BaseController {
	...
	public function paypalDonate($donate) {
		$donation = Donation::find($donate);
		if($donation->paypal_transaction_id) {
			return Redirect::action('DonationController@getIndex')->with('success','Already paid with PayPal');
		}
		$amount = floatval($donation->donated_amount);
		$date = date('Y-m-d');
		...
		$params = [
			...
			'currency' => 'PHP',
			'returnUrl' => URL::action('DonationController@paypalReturn', ['donate'=>$donate]),
	...
	public function paypalReturn($donate) {
		$donation = Donation::find($donate);
		$amount = floatval($donation->donated_amount);
		$date = date('Y-m-d');
		$remarks = $donation->remarks;
